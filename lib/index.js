...
function matcher(cols){
  ...
  keys.forEach(function(key){
    ...
    opts.pattern = opts.pattern instanceof Array ? opts.pattern : [opts.pattern];
    opts.pattern.forEach(function(glob){
      matchers[key].push(new Matcher(glob));
    });
  });
  ...
  return function(file, data){
    ...
    for (var key in matchers){
      var matched = matchers[key].every(function(glob) {
        return glob && glob.match(file);
      });
