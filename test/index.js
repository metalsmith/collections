  ...
  it('should take an array of patterns', function(done){
    ...
    metalsmith
      ...
      .build(function(err, files){
        if (err) return done(err);
        assert.equal(1, metalsmith.metadata().blogs.length);
        assert.equal(1, metalsmith.metadata().pages.length);
        assert.equal(files['three.md'].collection, 'blogs');
        assert.equal(files['four.md'].collection, 'pages');
        done();
      });
  });
